...
Using an LLM agent can save you time on some subtasks and streamline work by eliminating some delays. You can consistently achieve double-digit time savings by using LLM agents, depending on the task. When viewing a task as a multi-step workflow, the greatest time savings from an LLM agent occur during researching, drafting, and reviewing. Using AI throughout these steps significantly reduces the total time required for each task.
...
* Doing all or more your work on one task within one prompt session makes the LLM agent more helpful because then it is working with more task-specific context that it can build its prompt responses on.
...
* Similarly to what can happen during human peer reviews, an LLM agent cannot read between the lines that you wrote and will miss implied context of which you are aware as an author. If you sense that this is the case in its feedback or suggestion, you have two options how to proceed: you can start asking it why it "thinks" this way, or you can take over the draft as a human to make your own edits and then give the updated draft back to it. If you explain to it what you are trying to achieve or what its analysis is missing, it can radically change its previous assessment of a piece of text that you drafted.
